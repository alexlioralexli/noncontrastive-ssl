...
from main_simsiam import ProgressMeter, AverageMeter, save_checkpoint, get_incremental_dataloader
from autolearn.moco.data import moco_imagenet_train

import autolearn.moco.builder as moco_builder
import autolearn.byol.builder as byol_builder
import autolearn.moco.optimizer as moco_optimizer
...
parser.add_argument('--alg', default='moco', type=str)
...
def main():
    ...
    assert args.n_warmup_steps == 0  # not implemented yet
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.gpu == 0:
        ...
        wandb.init(config=vars(args), id=f'{exp_name}_{args.exp_id:04d}', project=args.alg, resume=not args.no_resume,
    ...
    if args.alg == 'moco':
        model = moco_builder.MoCo_ResNet(
            ...
            args.moco_dim, args.moco_mlp_dim, args.moco_t, distributed=args.distributed)
    elif args.alg == 'byol':
        model = byol_builder.BYOL_ResNet(
            partial(torchvision_models.__dict__[args.arch], zero_init_residual=True),
            args.moco_dim, args.moco_mlp_dim, distributed=args.distributed)
    else:
        raise NotImplementedError('Unrecognized SSL algorithm')
    ...
    train_dataset = moco_imagenet_train(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_stats = train(train_loader, model, optimizer, scaler, epoch, args)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, filename='checkpoint_%04d.pth.tar' % epoch, folder=wandb.run.dir)
